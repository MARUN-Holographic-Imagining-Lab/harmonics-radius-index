...
if __name__ == "__main__":
    ...
    original_image = cv2.imread(
        HIGH_RES_PATH, cv2.IMREAD_GRAYSCALE)  # pylint: disable=no-member
    bicubic_image = cv2.imread(
        BICUBIC_RES_PATH, cv2.IMREAD_GRAYSCALE)  # pylint: disable=no-member
    hat_image = cv2.imread(
        HAT_RES_PATH, cv2.IMREAD_GRAYSCALE)  # pylint: disable=no-member
    blacked_image = cv2.imread(
        BLACKED_RES_PATH, cv2.IMREAD_GRAYSCALE)  # pylint: disable=no-member
    ...
    bicubic_ssim = ssim(original_pha, bicubic_pha,
                        data_range=radians_data_range)
    bicubic_psnr = psnr(original_pha, bicubic_pha,
                        data_range=radians_data_range)
    ...
    blacked_ssim = ssim(original_pha, blacked_pha,
                        data_range=radians_data_range)
    blacked_psnr = psnr(original_pha, blacked_pha,
                        data_range=radians_data_range)

    original_ssim = ssim(original_pha, original_pha,
                         data_range=radians_data_range)
    original_psnr = psnr(original_pha, original_pha,
                         data_range=radians_data_range)
